...
# => Help information
usage() {
  echo "Purpose: 将SD卡重新分区（一个FAT32格式的分区、一个EXT4格式的分区）"
...
case $choice in
  y|Y)
    ...
    if [ $( command -v ${sudo_cmd} parted) ]; then
      ...
      ${sudo_cmd} parted -s /dev/${DISK} mkpart primary 0 100MiB
      # Create a partition from where we left off to the end of the disk
      ${sudo_cmd} parted -s /dev/${DISK} mkpart primary 100MiB 100%
      echo ""

      # Format the partition
      print_info "正在格式化分区..."
      ${sudo_cmd} mkfs.vfat -F 32 -n BOOT /dev/${DISK}${PART}1
      ${sudo_cmd} mkfs.ext4 -T largefile -F -L ROOTFS /dev/${DISK}${PART}2
      echo ""
